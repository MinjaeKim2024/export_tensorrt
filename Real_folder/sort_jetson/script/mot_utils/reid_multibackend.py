...
from backbones import build_model, get_nr_classes
from mot_utils.model_descript import (get_model_name,
                                                  get_model_url,
                                                  load_pretrained_weights,
                                                  show_downloadable_models)
from utils import logger as LOGGER
from mot_utils.checks import TestRequirements
...
def check_suffix(file="osnet_x0_25_msmt17.pt", suffix=(".pt",), msg=""):
    ...
    if file and suffix:
        ...
        for f in file if isinstance(file, (list, tuple)) else [file]:
            ...
            if "tar" in s:
                print("tar type weights are loaded from the directory")
            elif len(s):
                try:
                    assert s in suffix
...
class ReIDDetectMultiBackend(nn.Module):
    ...
    def __init__(
        self, weights="osnet_x0_25_msmt17.pt", device=torch.device("cpu"), fp16=False, custom = False, num_classes = 751
    ):
        super().__init__()
        ...
        if w.suffix == ".pt":
            ...
            elif file_exists(w) and "tar" in w.suffix :
                print("File is a tar file and exists in the directory")
            else:
                LOGGER.error(
        ...
        self.model = build_model(
            model_name,
            num_classes = num_classes if custom else get_nr_classes(w), # if custom, num_classes is the number of classes in the custom model
            pretrained=not (w and w.is_file()), # if w is a file, it is already pretrained
            use_gpu=device
        )
        device = torch.device("cuda:0")
        if self.pt and not self.engine and not self.onnx:  # PyTorch
            # populate model arch with weights
            if w and w.is_file() and w.suffix == ".pt":
                load_pretrained_weights(self.model, w)
                pass
                # it is necessary, but our weight name is customized
            print("Loading weights from ", w)                
            ...
            with open(w, 'rb') as f:
                engine_data = f.read()
            self.model_ = runtime.deserialize_cuda_engine(engine_data)
